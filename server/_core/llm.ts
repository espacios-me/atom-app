...
const assertApiKey = () => {
  if (!ENV.forgeApiKey && !ENV.geminiApiKey) {
    throw new Error("No API key configured (OPENAI_API_KEY or GEMINI_API_KEY)");
  }
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  // Use Gemini API if available, otherwise fall back to Forge API
  const useGemini = ENV.geminiApiKey && !ENV.forgeApiKey;

  if (useGemini) {
    return invokeGeminiLLM({
      messages,
      tools,
...
async function invokeGeminiLLM(params: {
  messages: Message[];
  tools?: Tool[];
  toolChoice?: ToolChoice;
  outputSchema?: OutputSchema;
  ...
  const geminiMessages = messages.map((msg) => ({
    role: msg.role === "assistant" ? "model" : msg.role === "user" ? "user" : msg.role,
    parts: [
      {
  ...
  // Transform Gemini response to match InvokeResult format
